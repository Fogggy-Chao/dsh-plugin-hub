...
// Profile packages need not have a private copy of the global host CLI.
let hostManifest
try { hostManifest = require.resolve('@deepseek-ai/dsh/package.json') }
catch { hostManifest = createRequire(realpathSync(process.argv[1])).resolve('@deepseek-ai/dsh/package.json') }
const hostRequire = createRequire(hostManifest)
const { resolveDshHome } = await import(hostRequire.resolve('@deepseek-ai/dsh-home-paths'))
...
export function runInstaller(profile, spec, home = resolveDshHome()) {
  return new Promise((resolve, reject) => {
    const bin = path.join(path.dirname(hostManifest), 'lib/bin.js')
    const child = spawn(process.execPath, [bin, 'plugin', '--profile', profile, 'add', spec, '--ignore-scripts', '--save-exact', '--registry=https://registry.npmjs.org'], { env: { ...process.env, DSH_HOME: home }, stdio: ['ignore', 'pipe', 'pipe'], detached: process.platform !== 'win32' })
...
export function createMarketplace(profile, { install = runInstaller, fetchManifest = async name => {
...
}, home = resolveDshHome(), loaded = () => false, restartSupported = false, lifecycle } = {}) {
  ...
  return {
    ...
    async install(id) {
      if (!enabled) throw new Error('Installation requires a valid Harness home and profile.')
      const item = catalog.plugins.find(p => p.id === id)
      ...
      Promise.resolve().then(async () => {
        ...
        await install(profile, `${item.package}@${manifest.version}`, home)
